...
import { CoachingResourcesServiceProxy, CreateCoachingResourceDto, FileParameter, DocumentType, CoachingDto, CoachingResourceType } from '@shared/service-proxies/service-proxies';
...
export class UploadPresentationMaterialComponent extends AppComponentBase implements OnInit {
  ...
  model = new CreateCoachingResourceDto();
  ...
  onTypeClick(type: CoachingResourceType): void {
    this.model.type = type;
    if (type === CoachingResourceType.Slides) {
  ...
  onFormSubmit(): void {
    ...
    this._coachingResourcesService.create(this.model)
      .pipe(takeUntil(this.destroyed$))
      .subscribe(response => {
        this._uploadService.upload(this.document.data, DocumentType.CoachingResource, response.id)
          .pipe(
