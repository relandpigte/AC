...
public class DashboardStudentAndTutor {
	...
	private static Tab courseTab = new Tab("Course", By.xpath("//a[@id='courses-tab']"));
	private static Element dashboardHeader = new Element("Dashboard", By.xpath("//h6[contains(text(),'Dashboard')]"));
	private static Element profileSummaryWidget = new Element("Profile widget",
			By.xpath("//app-profile-summary//div[@class='card']"));
	private static Element verificationWidget = new Element("Verification widget",
			By.xpath("//app-verifications//div[@class='card']"));
	private static Element verifyPhone = new Element("Verify phone",
			By.xpath("//app-verifications//h5[text()='Phone']//following::a[text()='Verify']"));
	private static Element phone = new Element("Verification Phone",
			By.xpath("//app-verifications//h5[text()='Phone']"));
	private static Element email = new Element("Verification email",
			By.xpath("//app-verifications//h5[text()='Email']"));
	private static Button createNewProject = new Button("Create new project",
			By.xpath("//a[contains(text(),'Create New Project')]"));
	private static Element recentProjectTitle = new Element("Recent project",
			By.xpath("//app-recent-projects//h4[text()='Recent Projects']"));
	private static Element recentActivityTitle = new Element("Recent activity",
			By.xpath("//app-recent-activity//h4[contains(text(),'Recent Activity')]"));
	...
	public static class OverviewTab {
		...
		public static class RecentProject {
			private static Element projectName(String projectname) {
				return new Element(projectname,
						By.xpath("//h4[text()='Recent Projects']/following::a[text()='" + projectname + "'][1]"));
			}
		...
		public static class ProfileWidget {
			...
			private static Element profilePhoto = new Element("Profile photo",
					By.xpath("//div[@class='card']//img[@alt='profile']"));
			private static Element coverPhoto = new Element("Cover photo",
					By.xpath("//div[@class='card']//img[@class='card-img-top']"));

			private static Element schoolName(String schoolName) {
				return new Element(schoolName,
						By.xpath("//app-profile-summary//p[contains(text(),'" + schoolName + "')]"));
			}

			private static Element badgeStatus(String badge) {
				return new Element(badge + " Badge", By.xpath(
						"//app-profile-summary//span[contains(@class,'badge')and contains(text(),'" + badge + "')]"));
			}

			private static Button becomeTutor = new Button("Become a tutor",
					By.xpath("//app-profile-summary//button[contains(text(),'Become a Tutor')]"));
	...
	public static class UsageTab {
		...
		private static Element totalHours = new Element("Total Hours",
				By.xpath("//a[@id='usage-tab' and contains(@class,'active')]/following::h6[text()='Total Hours']"));
		private static Element totalProject = new Element("Total project",
				By.xpath("//a[@id='usage-tab' and contains(@class,'active')]/following::h6[text()='Total Projects']"));
		private static Element chart = new Element("Overvoew chart", By.xpath(
				"//a[@id='usage-tab' and contains(@class,'active')]//following::canvas[contains(@class,'chartjs-render-monitor')]"));
