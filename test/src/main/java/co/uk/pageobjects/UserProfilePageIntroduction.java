...
public class UserProfilePageIntroduction {
	...
	public static class LanguageSpoken {
		...
		private static Button edit = new Button("Edit",
				By.xpath("//app-spoken-languages//i[contains(@class,'edit')]/parent::Button"));

		private static Element languageAndProficiency(String language, String proficiency) {
			return new Element("Language: " + language + ", Proficiency: " + proficiency,
					By.xpath("//app-spoken-languages//h5[contains(text(),'" + language
							+ "')]/following::h6[contains(text(),'" + proficiency + "')]"));
		}
		...
		public static class LanguageModal {
			...
			private static ListBox englishProficiency = new ListBox("English proficeieny",
					By.xpath("//select[@id='englishProficiency']"));
			private static Button addLanguage = new Button("Add language",
					By.xpath("//a[contains(text(),'Add Language')]"));

			private static Element otherlanguageAndProficieny(String language, String proficiency) {
				return new Element("Language: " + language + ", Proficiency: " + proficiency, By.xpath("//td[text()='"
						+ language + "']/following-sibling::td[contains(text(),'" + proficiency + "')]"));
			}

			private static Button save = new Button("Save",
					By.xpath("//app-edit-spoken-language-form//button[contains(text(),'Save')]"));

			private static Button editOtherLanguage(String language) {
				return new Button(language + " edit", By.xpath(
						"//td[text()='" + language + "']/following::span[contains(@class,'edit')]/parent::button"));
			}

			private static Button removeOtherLanguage(String language) {
				return new Button(language + " edit", By.xpath(
						"//td[text()='" + language + "']/following::span[contains(@class,'trash')]/parent::button"));
			}
			...
			public static class AddEditLanguageModal {
				...
				private static ListBox languageDropdown = new ListBox("Language",
						By.xpath("//select[@id='newOtherSpokenLanguage']"));
				private static ListBox proficiencyDropdown = new ListBox("Proficiency",
						By.xpath("//select[@id='newProficiency']"));
